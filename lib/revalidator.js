(function (exports) {
  ...
  /** casts function which can be used at `cast` attribute */
  validate.casts = {
  };
  ...
  function validateProperty(object, value, property, schema, options, errors) {
    ...
    if (options.cast) {
      ...
      if (options.castSource && object[property] !== value) {
        object[property] = value;
      }
    }
    ...
    checkType(value, schema.type, function(err, type) {
      ...
      //schema casting
      if (schema.cast && errorsCountBeforeCheckType == errors.length) {
        if (type != 'array' && type != 'object') {
          var casts = isArray(schema.cast) ? schema.cast : [schema.cast];
          for (var i = 0, l = casts.length; i < l; i++) {
            var cast = null;
            if (isString(casts[i])) {
              if (casts[i] in validate.casts) {
                cast = validate.casts[casts[i]];
              } else {
                return error('casts', property, casts[i], schema, errors, 'unknown cast: %{actual}');
              }
            } else if (isFunction(casts[i])) {
              cast = casts[i];
            } else {
              return error('casts', property, typeof casts[i], schema, errors, 'bad cast type: %{actual}');
            }
            try {
              object[property] = cast(object[property]);
            } catch(err) {
              return error('casts', property, err, schema, errors, 'error during exec cast function: %{actual}');
            }
          }
        } else {
          return error('casts', property, type, schema, errors, 'bad property type for casting: %{actual}');
        }
      }
    });
  };
