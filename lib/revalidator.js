          ...
          for (var i = 0, l = casts.length; i < l; i++) {
            ...
            if (isString(casts[i])) {
              if (casts[i] in validate.casts) {
                if (isFunction(validate.casts[casts[i]])) {
                  cast = validate.casts[casts[i]];
                } else {
                  //TODO: `cast not a function` error
                }
              } else {
                //TODO: `unknown cast` error
              }
            } else if (isFunction(casts[i])) {
              cast = casts[i];
            } else {
              //TODO: `bad cast type` error
            }
            if (cast) {
              object[property] = cast(object[property]);
            } else {
              //TODO: `unrecognized cast` error
            }
          }
        } else {
          //TODO: `bad property type for casting it` error
        }
      }
    });
  };
